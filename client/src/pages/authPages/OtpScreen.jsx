...
import { selectUserEmail, selectUserRole } from "../../redux/features/auth/authSlice.jsx"; // Redux selector to get user email
...
const OtpScreen = () => {
    ...
    const handleVerifyOtp = async () => {
        ...
        try {
            ...
            const response = await client.post("/auth/verify-email",
                ...
                { role: role, otp: otpString },
                { headers: { Authorization: `Bearer ${token}` } },
            );
            if (response.data.success) {
                toast.success("OTP verified successfully! Please login!");
                navigate('/tutordashboard'); // Redirect to login screen
            } else {
    ...
    const handleResendOtp = async () => {
        ...
        try {
            const token = localStorage.getItem('authToken'); // Use localStorage instead of SecureStore
            const response = await client.post("/auth/resend-otp", { role: role }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            if (response.data.success) {
